...
import type { UpdateJobDto, EmploymentType, WorkMode, JobPriority, SalaryType } from '@/types/jobs';
...
export default function EditJobPage() {
  ...
  const [form, setForm] = useState<UpdateJobDto>({});
  ...
  useEffect(() => {
    if (job && !initialized) {
      setForm({
        ...
        hiringPriority: job.hiringPriority,
        ...
        targetHireDate: job.targetHireDate
          ? new Date(job.targetHireDate).toISOString().split('T')[0]
          : '',
      });
      setInitialized(true);
    }
  ...
  const set = (key: keyof UpdateJobDto, value: unknown) =>
    setForm((f) => ({ ...f, [key]: value }));

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload: UpdateJobDto = { ...form };
    (Object.keys(payload) as (keyof UpdateJobDto)[]).forEach((k) => {
  ...
  if (!job) {
    return <div className="text-center py-20 text-sm text-muted-foreground">Job not found.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={job.reqId}
        breadcrumbs={[
          { title: 'Dashboard', href: '/dashboard' },
          { title: 'Jobs', href: '/jobs' },
          { title: job.title, href: `/jobs/${id}` },
          { title: 'Edit' },
        ]}
      />
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
        ...
        {updateMutation.isError && (
          <p className="text-sm text-destructive">Failed to update job. Please try again.</p>
        )}
